...
def create_end(user_id: int, round_id:int, end: EndCreate, end_dal: EndDAL = Depends(get_end_dal)):
    return end_dal.create_end(round_id=round_id, end=end)

@router.put("/user/{user_id}/round/{round_id}/end/{end_id}", response_model=None)
def update_end(user_id: int, round_id: int, end_id: int, end: EndUpdate, end_dal: EndDAL = Depends(get_end_dal)):
...
def delete_end(user_id: int, round_id: int, end_id: int, end_dal: EndDAL = Depends(get_end_dal)):
