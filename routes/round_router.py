...
async def read_round_types(round_dal: RoundDAL = Depends(get_round_dal)):
...
async def get_rounds_by_user_id(user_id: int, skip: int = 0, limit: int = 100, round_dal: RoundDAL = Depends(get_round_dal)):
    async with async_session() as session:
        async with session.begin():
            round_dal = RoundDAL(session)
...
async def get_round_by_user_by_round(user_id: int, round_id: int, round_dal: RoundDAL = Depends(get_round_dal)):
    async with async_session() as session:
        async with session.begin():
            round_dal = RoundDAL(session)
