...
def get_bows_by_user(user_id: int, skip: int = 0, limit: int = 100, bow_dal: BowDAL = Depends(get_bow_dal)) -> List[Bow]:
...
def get_bow_by_user(user_id: int, bow_id: int, bow_dal: BowDAL = Depends(get_bow_dal)) -> Bow:
    return bow_dal.get_bow_by_user(user_id, bow_id)
        
@router.post("/user/{user_id}/bow", response_model=None)
def create_bow(user_id: int, bow: BowCreate, bow_dal: BowDAL = Depends(get_bow_dal)):
    return bow_dal.create_bow(user_id, bow)

@router.put("/user/{user_id}/bow/{bow_id}", response_model=None)
def update_bow(user_id: int, bow_id: int, bow: BowUpdate, bow_dal: BowDAL = Depends(get_bow_dal)):
...
def delete_bow(user_id: int, bow_id: int, bow_dal: BowDAL = Depends(get_bow_dal)):
