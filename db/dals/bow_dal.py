...
class BowDAL():
    ...
    def get_bow_types(self) -> List[BowType]:
        q = self.db_session.execute(select(BowType).where(BowType.active == True))
        return q.scalars().all()

    def get_bows_by_user(self, user_id: int, skip: Optional[int] = 0, limit: Optional[int] = 100) -> List[Bow]:
    ...
    def get_bow_by_user(self, user_id: int, bow_id: int) -> Bow:
        q = self.db_session.execute(
    ...
    def create_bow(self, user_id: int, bow: Bow) -> Bow:
        dt = datetime.now()
        q = insert(Bow)
    ...
    def update_bow(self, bow_id: int, user_id: int, bow: Bow):
    ...
    def delete_bow(self, bow_id:int, user_id: int):
